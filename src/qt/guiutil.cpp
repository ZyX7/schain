...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SCH address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no schain: URI
...
{
    // Convert schain:// to schain:
    //
    //    Cannot handle this later, because schain:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open schain.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Schain.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Schain.lnk
...
{
    return GetAutostartDir() / "schain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a schain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Schain\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the schain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add schain app to startup item list
